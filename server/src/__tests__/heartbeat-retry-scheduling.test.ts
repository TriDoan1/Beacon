...
describeEmbeddedPostgres("heartbeat bounded retry scheduling", () => {
  ...
  it("cancels queued issue runs and deferred wakeups when an issue is cancelled", async () => {
    ...
    const result = await heartbeat.cancelIssueRuns({
      companyId,
      issueId,
      reason: "Cancelled because the issue was marked cancelled",
    });

    expect(result.runs.map((run) => run.id)).toEqual([queuedRunId]);
    expect(result.wakeups.map((wakeup) => wakeup.id)).toEqual([deferredWakeupId]);
    ...
    expect(Object.fromEntries(wakeups.map((wakeup) => [wakeup.id, wakeup.status]))).toEqual({
      [queuedWakeupId]: "cancelled",
      [deferredWakeupId]: "cancelled",
    });
  });
